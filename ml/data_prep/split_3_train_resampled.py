...
def _iter_batches(parquet_path: str, batch_size: int, columns=None):
    """Yield the input parquet in pandas batches."""
        ...
        for fam in fams:
            ...
            if fam in keep_full:
                ...
                for row in sub.to_dict(orient="records"):
                    reservoirs[fam], seen[fam] = _reservoir_update(reservoirs[fam], target_n, row, seen[fam], rng)
...
def write_split(
    *,
    ...
    """Resample train, copy val/test, write report. Returns the report metadata."""
    ...
    # Preserve evaluation splits exactly so comparisons stay honest.
